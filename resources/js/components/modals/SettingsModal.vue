                        ...
                        <div class="grid gap-6 lg:grid-cols-2">
                            <div class="space-y-6">
                                <section class="space-y-2 pt-0">
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-gray-500">Pronósticos de restock
                                        </p>
                                        <p class="text-[11px] text-gray-500">Define cómo se alimentan el widget de
                                            restock y los reportes condensados.</p>
                                    </div>
                                    <div class="flex flex-wrap gap-2">
                                        <label v-for="opt in availableHorizons" :key="opt"
                                            ...
                                            <input type="checkbox" class="hidden" :checked="selected.includes(opt)"
                                                @change="toggle(opt)" />
                                            {{ horizonLabels[opt] }}
                                        </label>
                                    </div>
                                    <p class="text-[11px] text-gray-500">
                                        Los horizontes seleccionados se ejecutan diariamente.<span v-if="lastRun"
                                            class="block text-gray-400">Última ejecución: {{ lastRun }}</span>
                                    </p>
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 text-xs text-gray-600">
                                        <div>
                                            <span class="block font-semibold text-gray-700">Inventario mínimo
                                                (días)</span>
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 text-xs text-gray-600">
                                        ...
                                        <p class="max-w-sm text-[11px] text-gray-500">
                                            Periodo histórico mínimo que se usará para calcular ventas; si las tablas
                                            actuales no cubren ese rango, se complementa automáticamente con las tablas
                                            históricas.
                                        </p>
                                    </div>
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button"
                                            ...
                                            :disabled="running || loading" @click="runForecast">
                                            <span v-if="running">Ejecutando…</span>
                                            <span v-else>Ejecutar pronóstico ahora</span>
                                        </button>
                                        <button type="button"
                                            class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-1.5 font-medium hover:bg-gray-50"
                                            :disabled="runningAutoClose || loading" @click="runAutoCloseCashbox">
                                ...
                                <section class="space-y-2 pt-4">
                                    <div>
                                        ...
                                        <p class="text-[11px] text-gray-500">Aplicado en reportes de caja y cálculo de
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[11px] text-gray-500">Porcentaje descontado cuando una venta
                                            se paga con tarjeta.</span>
                                    </div>
                                </section>
                                ...
                                <section class="flex flex-wrap items-center gap-2 text-xs text-gray-500 pt-4">
                                    <button type="button"
                                        class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-1.5 font-medium hover:bg-gray-50"
                                        :disabled="saving || loading" @click="saveSettings">
                                        <span v-if="saving">Guardando…</span>
                                        <span v-else>Guardar configuración</span>
                                    </button>
                                    <span class="text-[11px] text-gray-500">Los cambios impactan inmediatamente los
                                        módulos mencionados.</span>
                                </section>
                            ...
                            <div class="space-y-6 lg:pl-4">
                                <section class="space-y-2 pt-4">
                                    <div>
                                        ...
                                        <p class="text-[11px] text-gray-500">Usado en la pestaña “Importes
                                            recomendados”.</p>
                                    </div>
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <label class="text-xs text-gray-600">
                                            <span class="block font-semibold text-gray-700">Porcentaje sobre ventas
                                                (%)</span>
                                            <input type="number" step="0.1" min="0" max="100"
                                    ...
                                    <p class="text-[11px] text-gray-500">
                                        Controla el porcentaje y ventana de tiempo usados para sugerir nuevos importes
                                        en el módulo de Análisis.
                                    </p>
                                </section>
                                <section class="space-y-3 pt-4">
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-gray-500">Recalcular cargos
                                            tarjeta</p>
                                        <p class="text-[11px] text-gray-500">Ejecuta el rebalanceo de cargos de tarjeta
                                            para un día completo o una venta específica.</p>
                                    </div>
                                    <div class="grid gap-3 sm:grid-cols-2 text-xs text-gray-600">
                                        ...
                                        <label class="flex flex-col gap-1">
                                            ...
                                            <input type="number" min="1"
                                                class="rounded border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:ring-gray-900"
                                                v-model="rebalanceVentaId" placeholder="Ej. 22" />
                                        </label>
                                    </div>
                                    <p class="text-[11px] text-gray-500">
                                        Ingresa al menos uno de los dos campos. Si solo indicas el ID de venta, se usará
                                        la fecha de esa venta.
                                    </p>
                                    <button type="button"
                                        class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-1.5 font-medium hover:bg-gray-50"
                                        :disabled="runningRebalance || loading" @click="runCardRebalanceManual">
                                        <span v-if="runningRebalance">Recalculando…</span>
                                        <span v-else>Recalcular cargos tarjeta</span>
                                    </button>
                                    <div v-if="rebalanceLogText"
                                        class="rounded border border-gray-200 bg-gray-50 px-3 py-2 text-[11px] text-gray-700 whitespace-pre-wrap">
                                        {{ rebalanceLogText }}
                                    </div>
                                </section>

                                <!-- Tabla de cambios aplicados removida -->
                                ...
                                <div v-if="message"
                                    class="rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700">
                                    {{ message }}
                                </div>
                            </div>
                        </div>
